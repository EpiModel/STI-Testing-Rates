...
# compare with grouping all others together

dt <- dplyr::select(d, sti.trate.all, city)
dt <- dt[complete.cases(dt), ]
dim(dt)
...
fit.city2b <- stan(file = "models/negbin-glm.city.mlm.stan",
                 data = data,
                 chains = 4,
                 cores = 4,
                 iter = 6000,
...
df2b <- as.data.frame(fit.city2b, par = "a")
colnames(df2b) <- levels(as.factor(dt$city))
df2b <- exp(df2b)/2
df2b <- select(df2b, -starts_with("zOther"))
dfg2b <- gather(df2b)
head(dfg2b)
table(dfg2b$key)

ggplot(dfg2b, aes(key, value)) +
  geom_boxplot(fill = "steelblue2", outlier.alpha = 0, fatten = 0.75) +
  scale_y_continuous(limits = c(0.5, 2.0)) +
  theme_bw()

t(round(apply(df2b, 2, function(x) quantile(x, probs = c(0.5, 0.025, 0.975))), 3))
...
save(t2.fit, file = "t2.fit.rda")

ages <- 30
pnum <- 0:25
pred <- exp(-0.67437 + 0.07104*ages + -0.00100*(ages^2) + 0.01542*pnum)/2
pred

plot(pred)
lines(pred)

df <- as.data.frame(t2.fit, par = c("a", "beta"))
